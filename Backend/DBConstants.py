class DBConstants:
    register_user     = 'INSERT INTO Utilizador (email,password,idCarteira,dataNascimento,nif,isAdmin,isEspecialista) VALUES ((%s),(%s),(%s),(%s),(%s),(%s),(%s));'
    add_wallet        = 'INSERT INTO Carteira (saldoCarteira) VALUES(0.00);'
    get_log_info      = 'SELECT email, password, isAdmin, isEspecialista FROM Utilizador WHERE email=(%s);'
    add_team          = 'INSERT INTO EquipasPorJogo(nomeEquipa, idJogo, Odd, jogaEmCasa) VALUES((%s),(%s),(%s),(%s));'    
    get_sports        = 'SELECT DISTINCT nomeDesporto FROM Jogo;'
    get_by_sport      = 'SELECT idJogo FROM Jogo WHERE nomeDesporto = (%s);'
    get_game_info     = 'SELECT nomeEquipa, Odd, jogaEmCasa FROM EquipasPorJogo WHERE idJogo = (%s);'
    get_balance       = 'SELECT saldoCarteira FROM Carteira WHERE idCarteira = (SELECT idCarteira FROM Utilizador WHERE email=%s);'
    create_promotion  = 'INSERT INTO Promoção(idJogo, percentagemAumento) Values(%s,%s);'
    update_odds       = 'UPDATE EquipasPorJogo SET Odd = Odd * %s WHERE idJogo=%s;'
    get_promotion     = 'SELECT idPromoçao, percentagemAumento, idJogo FROM Promoção WHERE idPromoçao=%s'
    remove_promotion  = 'DELETE FROM Promoção WHERE idPromoçao=%s;'
    get_history_bets  = 'SELECT idAposta, dataAposta, valorApostado FROM Aposta WHERE emailConta=%s;'
    get_history_trans = 'SELECT idTransação, dataTransação, saldoAntes, valorTransação FROM Transação WHERE emailConta=%s;'
    reg_transaction   = 'INSERT INTO Transação(saldoAntes,valorTransação,emailConta) VALUES(%s,%s,%s)'
